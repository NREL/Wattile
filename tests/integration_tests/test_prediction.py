...
from wattile.models import AlfaEnsembleModel, AlfaModel, BravoModel
...
def test_prediction_alfa_ensemble(config_for_tests, tmpdir):
    # don't run training
    ...
    data = read_dataset_from_file(config_for_tests)
    _, val_df = prep_for_rnn(config_for_tests, data)
    model = AlfaEnsembleModel(config_for_tests)
    results = model.predict(val_df)
