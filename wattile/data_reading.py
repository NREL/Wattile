...
def _get_dataset_config(configs):
    ...
    :rtype: pd.DataFrame
    """
    dataset_dir = Path(configs["data_handling"]["data_dir"])
    configs_file_inputdata = dataset_dir / configs["data_handling"]["data_config"]
...
def read_dataset_from_file(configs):
    ...
    timestamp_start = dt.datetime.fromisoformat(configs["data_handling"]["start_time"])
    timestamp_end = dt.datetime.fromisoformat(configs["data_handling"]["end_time"])
    ...
    data_full_p = _concat_data_from_files(
        ...
        needed_columns=configs["data_handling"]["predictor_columns"],
    )

    # read in target data
    target_data_info = df_inputdata[df_inputdata.contentType == "targets"]
    data_full_t = _concat_data_from_files(
        target_data_info.path, needed_columns=[configs["data_handling"]["target_var"]]
    )
    ...
    if configs["use_case"] == "prediction":
        ...
        data_full[configs["data_handling"]["target_var"]] = -999
    else:
