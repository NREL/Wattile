...
class AlfaModel(AlgoMainRNNBase):
    def data_iterable_random(
        ...
        if run_train:
            ...
            X_train = train_data.drop(
                self.configs["data_handling"]["target_var"], axis=1
            ).values.astype(dtype="float32")

            # Output variable
            y_train = train_data[self.configs["data_handling"]["target_var"]]
        ...
        y_val = val_data[self.configs["data_handling"]["target_var"]]
    ...
    def test_processing(
        ...
        with torch.no_grad():
            ...
            if transformation_method == "minmaxscale":
                final_preds = (
                    (
                        train_max[self.configs["data_handling"]["target_var"]]
                        - train_min[self.configs["data_handling"]["target_var"]]
                    )
                    * semifinal_preds
                ) + train_min[self.configs["data_handling"]["target_var"]]
                final_targs = (
                ...
                final_preds = (
                    semifinal_preds
                    * train_std[self.configs["data_handling"]["target_var"]]
                ) + train_mean[self.configs["data_handling"]["target_var"]]
                final_targs = (
                    semifinal_targs
                    * train_std[self.configs["data_handling"]["target_var"]]
                ) + train_mean[self.configs["data_handling"]["target_var"]]
        ...
        for epoch in epoch_range:
            ...
            for i, (feats, values) in enumerate(train_loader):
                ...
                if n_iter % self.configs["eval_frequency"] == 0:
                    ...
                    # val_df[self.configs["data_handling"]["target_var"]])),
                    #     max(max(predictions),
                    # max(val_df[self.configs["data_handling"]["target_var"]])),
                    #     5,
                    # )
    ...
    def run_prediction(
        ...
        if transformation_method == "minmaxscale":
            ...
            final_preds = (
                semifinal_preds * train_std[self.configs["data_handling"]["target_var"]]
            ) + train_mean[self.configs["data_handling"]["target_var"]]
    ...
    def _plot_results(self, targets, predictions):
        ...
        ax1.set_ylabel(self.configs["data_handling"]["target_var"])
        ax1.legend()
        plt.show()
        # fig.savefig(
        #     os.path.join(
