...
def timelag_predictors(data, configs):
    ...
    data[target_var] = target.shift(freq="-" + window_width_futurecast).copy()

    # drop all nans
    data = data.dropna(how="any")

    # adjust time index to match the EC values
    data.index = data.index.shift(freq=window_width_futurecast).copy()
...
def timelag_predictors_target(data, configs):
    ...
    data[target_var] = data[target_var].shift(freq="-" + window_width_futurecast).copy()
