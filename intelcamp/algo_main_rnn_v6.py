...
def data_iterable_random(train_data, val_data, run_train, train_batch_size, val_batch_size, configs):
    ...
    if run_train:
        ...
        # Output variable (batch*72), then (batch*(72*5))
        ...
        y_train = np.repeat(np.expand_dims(y_train, axis=2), repeats=len(configs['qs']), axis=2)
    ...
    y_val = np.repeat(np.expand_dims(y_val, axis=2), repeats=len(configs['qs']), axis=2)
...
def process(train_loader, val_loader, val_df, num_epochs, run_train, run_resume, writer, transformation_method,
    ...
    target_len = (configs["S2S_stagger"]["initial_num"] + configs["S2S_stagger"]["secondary_num"])
    ...
    if run_train:
        ...
        # If you want to start training a model from scratch
            ...
            if configs["arch_type_variant"] == "lstm":
                model = seq2seq_model.lstm_seq2seq(input_size=input_dim, hidden_size=hidden_dim)

                # model = seq2seq_model.LSTM_Model(input_dim, hidden_dim, layer_dim, output_dim)
        ...
        for epoch in epoch_range:
            ...
            for i, (feats, values) in enumerate(train_loader):
                ...
                # outputs tensor
                outputs = torch.zeros(train_batch_size, target_len, 1)

                # initialize hidden state
                encoder_hidden = model.encoder.init_hidden(train_batch_size)
                ...
                # encoder outputs
                encoder_output, encoder_hidden = model.encoder(features)

                # decoder with teacher forcing
                decoder_input = features[:, -1, :]  # shape: (batch_size, input_size)
