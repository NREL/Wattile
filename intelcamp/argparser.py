...
def get_arguments():
    ...
    parser.add_argument(
        "--tr-batch-size", default=1000, type=int, help="train batch size"
    )
    parser.add_argument(
        "--te-batch-size", default=200, type=int, help="train batch size"
    )
    parser.add_argument(
        "--resume",
    ...
    parser.add_argument(
        "--tr-exp-num", help="train experiment number for the given architecture type"
    )
    parser.add_argument(
        "--te-exp-num", help="test experiment number for the given architecture type"
    )
    parser.add_argument(
        "--hidden-nodes", help="number of hidden nodes in each hidden layer"
    )
    parser.add_argument("--weight-decay", help="lambda for regularization")
    ...
    if args.arch_type.lower() in ["ffnn", "rnn", "lstm", "gru"]:
        ...
        print(
            "Architecture type is invalid. See help for types of architecture available. Exiting..."
        )
        parser.print_help()
        exit()
